...
export default function PackageDetailScreen({ route, navigation }) {
    ...
    const { packages, updatePackageStatus, addPackage } = usePackages();

    const pkg = packages.find(p => p.id === packageId);

    // Buscar todos los paquetes que comparten la misma direccion
    const addressPackages = pkg ? packages.filter(p => p.address === pkg.address && p.status !== 'entregado' && p.status !== 'rechazado') : [];
    ...
    const isPickedUp = addressPackages.every(p => p.status === 'recogido') || addressPackages.length === 0;

    // Verificar si están todos (los no recogidos) listos para registrarse
    const allProcessed = addressPackages.filter(p => p.status !== 'recogido').every(p => photos[p.id] && scans[p.id]);
    ...
    const handleBarCodeScanned = ({ data }) => {
        if (!scanningPkgId) return;

        // Idealmente coincide con el ID esperado, pero si no, intentamos hacer auto-match
        const autoMatch = addressPackages.find(p => p.id === data || p.trackingNumber === data);
        if (autoMatch) {
            setScans(prev => ({ ...prev, [autoMatch.id]: true }));
            setScanningPkgId(null);
        } else {
            // El fallback funciona así en el escáner principal
            // MOCK/Fallback para que puedas probar el demo sin tener QRs reales:
            setScans(prev => ({ ...prev, [scanningPkgId]: true }));
            setScanningPkgId(null);
            Alert.alert("Código Escaneado", "Marcado como leído para demo.");
        }
    };

    const handleNewPackageScanned = ({ data }) => {
        // Verificar si ya existe en la ruta general para no duplicar datos si era de esta misma direccion
        const exists = addressPackages.find(p => p.id === data || p.trackingNumber === data);
        if (exists) {
        ...
        // Pre-aprobar el escaneo ya que lo acabamos de leer
    ...
    const handleNavigate = () => {
        if (pkg.address) {
            const url = `http://maps.apple.com/?q=${encodeURIComponent(pkg.address)}`;
            Linking.openURL(url);
        }
    };
        ...
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            <ScrollView style={styles.content} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
                <View style={styles.compactInfoSection}>
                    <View style={styles.addressRow}>
                        <Text style={styles.compactAddress} numberOfLines={2}>{pkg.address}</Text>
                ...
                <View style={styles.content}>
                    <View style={styles.packagesListHeader}>
                        <Text style={styles.packagesListTitle}>BULTO(S) A RETIRAR ({addressPackages.length})</Text>
                    </View>

                    {addressPackages.map((p, i) => {
                                ...
                                {!disabledRow && (
                                    <View style={styles.packageActionButtons}>
                                        ...
                                        >
                                            ...
                                            <Text style={[styles.pkgActionLabel, scans[p.id] && { color: '#059669' }]}>QR SCAN</Text>
                                        </TouchableOpacity>
                                    </View>
                                )}

                                {(photos[p.id] || p.photoUri) && (
                                    <View style={styles.photoPreviewRow}>
                                        <Image source={{ uri: photos[p.id] || p.photoUri }} style={styles.rowPreviewImage} />
                                        {!disabledRow && (
                                            <TouchableOpacity
            ...
            {!isPickedUp && (
                <View style={styles.fixedButtonsContainer}>
                    <TouchableOpacity
                        style={styles.addExtraFooterButton}
                        onPress={handleAddExtraPackage}
                    >
            ...
            {/* Modal Scanner */}
            <Modal visible={!!scanningPkgId || isAddingPackage} animationType="slide" presentationStyle="pageSheet">
                <View style={styles.modalContainer}>
                    <View style={styles.modalHeader}>
                        <Text style={styles.modalTitle}>{isAddingPackage ? "Escanear Nuevo Paquete" : "Escanear Paquete"}</Text>
                        <TouchableOpacity onPress={() => { setScanningPkgId(null); setIsAddingPackage(false); }}>
