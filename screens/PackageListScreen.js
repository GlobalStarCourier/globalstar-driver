...
export default function PackageListScreen({ navigation }) {
    const { packages, loading, fetchPackages, updatePackageStatus, addPackage } = usePackages();
    ...
    const routePackages = useMemo(() => {
        const activePackages = packages.filter(p => p.status !== 'entregado' && p.status !== 'rechazado');
    ...
    const handleScanButton = async () => {
        ...
        // Desbloquear al abrir el escaner
    ...
    const handleBarCodeScanned = ({ data }) => {
        ...
        // MOCK/Fallback: Si no encuentra, y la data no está vacía, tomamos el 1ero para probar
        if (!foundPkg && packages.length > 0) {
            foundPkg = packages[0];
        }
    ...
    const takePhotoFor = async (id) => {
        try {
            ...
            if (status !== 'granted') {
                Alert.alert('Permiso denegado', 'Necesitamos acceso a la cámara para la foto.');
                return;
            }
            ...
            Alert.alert("Error de Cámara", "Hubo un problema al abrir la cámara: " + error.message);
        }
    };

    const handleInnerBarCodeScanned = ({ data }) => {
        if (!scanningInnerPkgId) return;

        const autoMatch = scannedAddressPackages.find(p => p.id === data || p.trackingNumber === data);
        if (autoMatch) {
            setScans(prev => ({ ...prev, [autoMatch.id]: true }));
            setScanningInnerPkgId(null);
        } else {
            // MOCK/Fallback
            setScans(prev => ({ ...prev, [scanningInnerPkgId]: true }));
            setScanningInnerPkgId(null);
            Alert.alert("Código Escaneado", "Marcado como leído para demo.");
        }
    };

    const handleNewPackageModalScanned = ({ data }) => {
        const exists = scannedAddressPackages.find(p => p.id === data || p.trackingNumber === data);
        if (exists) {
            Alert.alert("Aviso", "Este paquete ya está en la lista.");
            setIsAddingPackage(false);
            return;
        }

        const newPkgId = `extra-${Date.now()}`;
        const newPkg = {
            id: newPkgId,
            trackingNumber: data,
            address: scannedPackage.address,
            customerName: scannedPackage.customerName,
            phone: scannedPackage.phone,
            status: 'pendiente'
        };
        addPackage(newPkg);
        setScans(prev => ({ ...prev, [newPkgId]: true }));
        setIsAddingPackage(false);
    };
    ...
    const handleRegisterPackage = () => {
        ...
        const addressPackages = packages.filter(p => p.address === scannedPackage.address && p.status !== 'entregado' && p.status !== 'rechazado');
        ...
        setPhotos({});
        setScans({});
        Alert.alert('¡Excelente!', `Se han marcado ${addressPackages.length} paquete(s) como Retirado.`);
    };

    const handleCall = () => {
        if (scannedPackage?.phone) {
            Linking.openURL(`tel:${scannedPackage.phone}`);
        } else {
    ...
    const handleNavigate = () => {
        if (scannedPackage?.address) {
            const url = `http://maps.apple.com/?q=${encodeURIComponent(scannedPackage.address)}`;
            Linking.openURL(url);
        }
    };
            ...
            >
                <View style={styles.cardMain}>
                    <View style={styles.cardHeader}>
                        ...
                        <Text style={[styles.addressText, isPickedUp && styles.textPickedUp]} numberOfLines={2}>
                            {item.address}
                        </Text>
                    </View>
    ...
    const scannedAddressPackages = useMemo(() => {
        if (!scannedPackage) return [];
        return packages.filter(p => p.address === scannedPackage.address && p.status !== 'entregado' && p.status !== 'rechazado');
    }, [scannedPackage, packages]);

    const allProcessed = scannedAddressPackages.filter(p => p.status !== 'recogido').every(p => photos[p.id] && scans[p.id]);
    ...
    return (
        <SafeAreaView style={styles.container} edges={['top']}>
            ...
            <Modal visible={isScanningQR} animationType="slide" presentationStyle="pageSheet">
                <View style={styles.modalContainer}>
                    <View style={styles.modalHeader}>
                        ...
                        <TouchableOpacity onPress={() => { setIsScanningQR(false); scanLock.current = true; }}>
                    ...
                    <View style={styles.resultCard}>
                        ...
                        <ScrollView style={{ width: '100%' }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
                            {scannedPackage && (
                                <View style={styles.scannedInfoContainer}>
                                    <Text style={styles.h1Address} numberOfLines={3}>{scannedPackage.address}</Text>
                                    ...
                                    <View style={styles.scannedInfoRow}>
                                        ...
                                        <Text style={styles.scannedInfoText}>{scannedPackage.phone || 'Sin número'}</Text>
                                    </View>

                                    <View style={styles.divider} />

                                    <View style={styles.packagesListHeader}>
                                        <Text style={styles.packagesListTitle}>Paquetes en esta dirección ({scannedAddressPackages.length}):</Text>
                                    </View>

                                    {scannedAddressPackages.map((p, i) => {
                                        const isProcessed = photos[p.id] && scans[p.id];
                                        const isDocPickedUp = p.status === 'recogido';
                                        const disabledRow = isDocPickedUp;

                                        const isSingle = scannedAddressPackages.length === 1;

                                        return (
                                            <View key={p.id} style={styles.packageDetailRow}>
                                                <View style={styles.packageInfoCol}>
                                                    <View style={styles.packageStatusIndicator}>
                                                        <Box color={(photos[p.id] && scans[p.id]) || isDocPickedUp ? "#059669" : "#DDD"} size={16} />
                                                        <Text style={[styles.packageTrackingText, ((photos[p.id] && scans[p.id]) || isDocPickedUp) && styles.textPickedUp]}>
                                                            {p.trackingNumber}
                                                        </Text>
                                                    </View>
                                                </View>
                                                ...
                                                {!disabledRow && (
                                                    <View style={styles.packageActionButtons}>
                                                        ...
                                                        >
                                                            ...
                                                            <Text style={[styles.pkgActionLabel, photos[p.id] && { color: '#059669' }]}>FOTO</Text>
                                                        </TouchableOpacity>
                                                        ...
                                                        >
                                                            ...
                                                            <Text style={[styles.pkgActionLabel, scans[p.id] && { color: '#059669' }]}>QR</Text>
                                                        </TouchableOpacity>
                        ...
                        <View style={styles.fixedButtonsContainer}>
                            ...
                            <TouchableOpacity
                                ...
                                onPress={() => { setShowRegisterModal(false); setScannedPackage(null); setPhotos({}); setScans({}); }}
                            >
            ...
            <Modal visible={!!scanningInnerPkgId || isAddingPackage} animationType="slide" presentationStyle="pageSheet">
                <View style={styles.modalContainer}>
                    <View style={styles.modalHeader}>
                        <Text style={styles.modalTitle}>
                            {isAddingPackage ? 'Escanear Nuevo Paquete' : 'Escanear Paquete'}
                        </Text>
                        <TouchableOpacity onPress={() => { setScanningInnerPkgId(null); setIsAddingPackage(false); }}>
                            <X color="#000000" size={28} />
                        </TouchableOpacity>
                    </View>
                    <View style={styles.cameraContainer}>
            ...
            <Modal visible={showIncidentModal} transparent animationType="slide" presentationStyle="overFullScreen">
                            ...
                            {incidentTypes.map((type) => (
                                <TouchableOpacity
                                    ...
                                    style={[
                                        ...
                                        selectedIncident === type.id && { borderColor: type.color, backgroundColor: type.color + '08' }
                                    ]}
                                    onPress={() => setSelectedIncident(type.id)}
                                >
                                    <View style={[styles.incidentIconCircle, { backgroundColor: type.color + '15' }]}>
                                        <type.icon color={type.color} size={20} />
                                    </View>
                                    <Text style={[styles.incidentOptionLabel, selectedIncident === type.id && { color: type.color }]}>
                                        {type.label}
                                    </Text>
                                </TouchableOpacity>
                            ))}
                        ...
                        <View style={styles.notifyCtn}>
                            <View style={styles.notifyInfo}>
                                ...
                                <Text style={styles.notifyText}>
                                    Al reportar, podemos enviar un mensaje automático de demora a tus {routePackages.filter(p => p.status === 'pendiente').length} destinos pendientes.
                                </Text>
                            </View>
                        </View>
...
const styles = StyleSheet.create({
    ...
    incidentTitleIcon: {
        width: 48, height: 48, borderRadius: 16, backgroundColor: '#FEF2F2',
